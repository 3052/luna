...
type MediaPlaylist struct {
   ...
   PlaylistType   string
   Segments       []*Segment
   Keys           []*SessionKey // Global keys or keys rotating
...
type Segment struct {
   ...
   Duration float64
   Title    string
   Key      *SessionKey // Encrypt key specific to this segment (if any)
}

// resolve updates the Segment's URI and its nested Key URI to be absolute.
